...
WORKSAPCE_ID = "prozorro_tenders"

WORKSPACE_PATH = f"projects/{PROJECT_ID}/locations/{REGION}/repositories/{REPOSITORY_ID}/workspaces/{WORKSAPCE_ID}"
...
@dag(
    ...
    descriptions = "Исполняющий DAG 2: компилирует и запускает трансформацию Dataform ProZorro (Core & Reporting витрины)",
)
def prozorro_dataform_transform_pipeline():
    ...
    invoke_dataform = DataformCreateWorkflowInvocationOperator(
        ...
        repository_id = REPOSITORY_ID,
        workflow_invocation = {
            "complite_result": "{{ task_instance.xcom_pull('compile_dataform')['name'] }}",
            "invocation_config": {
